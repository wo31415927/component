...
public class ExpectationTest {
  ...
  public void testExpectation(
    ...
    //此处声明class，表示对该class任意实例进行部分mock
    //也可指定指定具体的实例
    ...
    //会被mock
    //静态函数可以被mock
    Assert.assertEquals("mockStatic", Dependency1.mockStatic());
    //returns
    //@Mock影响该类的所有对象
    Assert.assertEquals("1", new Dependency1().mockMethod());
    Assert.assertEquals("2", new Dependency1().mockMethod());
    Assert.assertEquals("3", new Dependency1().mockMethod());
    //超出returns的对象取最后一个值
    Assert.assertEquals("3", new Dependency1().mockMethod());
    //Expectations未record，会被Jmockit处理并返回null或类型初始值
    Assert.assertNull(null, d1.random());
    //只会mock @Injectable声明的对象
    Assert.assertEquals("another", d2.mockMethod());
    Assert.assertEquals("real", new Dependency2().mockMethod());
    //@Injectable static函数不会被Jmockit处理
    Assert.assertEquals("static", Dependency2.mockStatic());
    //mock特定constructor产生的实例
    Assert.assertEquals("MockHello", new Dependency3("Hello").mockMethod());
    Assert.assertEquals("MockHello", new Dependency3("Hello").mockMethod());
    Assert.assertEquals("MockHello", new Dependency3("Hello").mockMethod());
    Assert.assertNull("", new Dependency3("Hello1").mockMethod());
    //
    Assert.assertEquals("paramString", new Dependency3("Hello").param("Haha", "", "", "yes"));
    Assert.assertEquals(null, new Dependency3("Hello").param("Haha", "abc", "", ""));
    //initPerson()在Expectations中未定义，会返会一个@Injectable的mock对象
    Assert.assertEquals(0, new Dependency3("Hello").initPerson1().getAge());
    //相同方法返回的是同一个@Injectable对象
    Assert.assertSame(d3.initPerson1(), d3.initPerson1());
    //不同方法返回的是不同的@Injectable对象,若声明@Person，则返回的是同一个@Mock对象
    Assert.assertNotSame(d3.initPerson1(), d3.initPerson2());
  }
