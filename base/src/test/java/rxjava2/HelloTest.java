...
public class HelloTest {
  /** HelloWorld Consumer Subscriber disposal开关 */
  @Test
  ...
  public void test1() throws Exception {
    Flowable.create(
        ...
        .observeOn(
            Schedulers.from(
                ExtraExecutors.renamingDecorator(
                    Executors.newSingleThreadExecutor(), () -> "Extra")))
        .
    ...
    Flowable.range(1, 2)
                    ...
                    .map(
                        new Function<Integer, Object>() {
                          ...
                          public Object apply(@NonNull Integer integer) throws Exception {
                            //这些异常log会打印的出来，即便后边对异常进行了捕捉
                            int i = 1 / 0;
                            return integer * integer;
                          }
                        }))

        //多线程map，doOnError由随机线程触发一次
        /*.doOnError(
        ...
        .onErrorResumeNext(
            new Function<Throwable, Publisher<? extends Integer>>() {
              @Override
              public Publisher<? extends Integer> apply(@NonNull Throwable throwable)
                  throws Exception {
                //异常发生时按原有值重新发射
        ...
        //main线程触发一次，之前的异常log，组件内部有时会打印出来，有时不会打印出来
        .blockingSubscribe(w -> log.info(w.toString()), ex -> log.error(ex.getMessage()));
    TimeUnit.SECONDS.sleep(1);
  }

  /**
   * 背压:是一种Flow Control策略，具体是生产太快,消费太慢时,由下游观察者通知上游的被观察者发送事件 Hot Observables: 订阅之后开始发送事件的Observable
   * Cold Observables: 创建之后开始发送事件的Observable Observable/Observer:这种模型不支持背压,当发生生产过快的场景时,event会堆积直到OOM
   */
  @Test
